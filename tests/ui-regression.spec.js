...
async function assertNoOverflow(page, label) {
  ...
  expect(
    size.scrollWidth,
    `${label}: horizontal overflow detected: document ${size.scrollWidth}px > viewport ${size.clientWidth}px (body ${size.bodyScrollWidth}px)`
  ).toBeLessThanOrEqual(size.clientWidth + 2);
}

async function assertLocalLinks(page, label) {
  const hrefs = await page.locator('a[href]').evaluateAll((nodes) =>
    [...new Set(nodes.map((a) => a.getAttribute('href')).filter(Boolean))]
  );
